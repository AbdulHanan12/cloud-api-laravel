...
<section id="pricing" class="relative py-20 bg-gray-50">
    <div class="relative px-10 mx-auto max-w-7xl lg:px-16">
        <div class="max-w-3xl mx-auto mb-16 text-center lg:mb-24">
            <h2 class="mt-3 mb-5 text-5xl font-medium font-heading">{{ __('wpbox.pricing_title')}}</h2>
            <p class="mb-5 text-xl text-gray-400">{{ __('wpbox.pricing_subtitle') }}</p>
        </div>
        <?php
        ...
        <div class="md:grid md:grid-cols-2 lg:grid-cols-{{$gridCount}} gap-x-10">
            ...
            @foreach ($plans as $keyplan =>  $plan)
                <div class="col-span-1 p-10 bg-white border-t-4 {{$colors[$keyplan]}}">
                    ...
                    <a href="{{ route('register') }}" class="flex items-center justify-center w-full h-12 mt-8 font-medium text-blue-600 border-2 border-blue-500 rounded-full">
                        {{ __('wpbox.start_now')}}
                    </a>
                </div>
            @endforeach
